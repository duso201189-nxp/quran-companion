...
void main() {
  ...
  group('getLearningStatistics', () {
    test('gộp SrsCard CẢ 2 loại (ayah + lemma) + tái dùng streak đọc thật',
      ...
      // AnalyticsRepositoryImpl.getLearningStatistics() không có cách
      // tiêm `today` xuống currentStreak() (khác _CountingStudySession
      // Repository ở trên, vốn CHO PHÉP tiêm nhưng chỉ dùng ở nhóm test
      // khác) — ghi phiên đọc cho NGÀY THẬT hôm nay thay vì hardcode 1
      // ngày cố định, để assertion streak >= 1 đúng bất kể chạy lúc nào,
      // cùng tinh thần "streak đọc thật" mà tên test đã nêu.
