  ...
  group('tên Surah chuẩn hóa', () {
    ...
    test('database khớp dataset Quran.com cho cả 114 tên', () async {
      ...
      for (final s in surahs) {
        final expected =
            (names['${s.id}'] as Map<String, dynamic>)['latin'];
    ...
    test('không còn dạng cũ kiểu Tanzil (Faatiha, Baqara, Naas)',
        () async {
      final surahs = await repo.getAllSurahs();
      const legacy = ['Faatiha', 'Baqara ', 'Naas', 'Aal-i', 'Maaida'];
      for (final s in surahs) {
        for (final old in legacy) {
          expect(s.nameLatin.contains(old), isFalse,
