...
void main() {
  ...
  group('chuẩn hóa phiên âm (toàn bộ 6236 ayah)', () {
    late AppDatabase db;
    ...
    test('định dạng sạch: không HTML/AA, không lỗi khoảng trắng, '
    ...
    test('quy tắc biên tập: không dấu \' ASCII, không nguyên âm dài '
      ...
      for (final entry in dataset.entries) {
        ...
        expect(doubled.hasMatch(v), isFalse,
            reason: '${entry.key} còn nguyên âm lặp: $v');
        for (final w in v.split(' ')) {
          if (w.contains('A')) {
            expect(w.contains('Allāh'), isTrue,
